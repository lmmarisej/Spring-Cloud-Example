 ...
 *
 * 服务注册的主要实现位于 AbstractlnstanceRegistry#registry 方法 中
 *
 */
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    //  统计了近 3 分钟内进 行注 册 、修改和剔除的服务实例信息  用于记录增量式注册表信息。
     ...
     * 服务注册
     *
     * 服务实例的 Instancelnfo 保存在 Lease 中， Lease 在 AbstractlnstanceRegistry 中统一通过 ConcurrentHashMap 保存在内存中。
     */
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        read.lock();    // 在服务注册过程中，会先获取 一 个读 锁，防止其他线程对 registry 注册表进行数据操作，避免数据的不 一致
        try {
            Map<String, Lease<InstanceInfo>> gMap = registry.get(registrant.getAppName());      // 根据 appName对服务实例集群进行分类
            REGISTER.increment(isReplication);
            if (gMap == null) {
                final ConcurrentHashMap<String, Lease<InstanceInfo>> gNewMap = new ConcurrentHashMap<String, Lease<InstanceInfo>>();
                // 这里有一个比较严谨的操作 ， 防止在添加新的服务实例集群租约时把已有的其他线程添加的集群 租约覆盖掉，如果存在该键值，直接返回已存在的值 ;
            ...
            gMap.put(registrant.getId(), lease);        // 保存租约
            //用来统计最近注册服务实例的数据
            ...
            //添加最近租约变更记录队列，标识 Action飞rpe为 ADDED
            recentlyChangedQueue.add(new RecentlyChangedItem(lease));
            registrant.setLastUpdatedTimestamp();       //设置服务实例信息更新时间
            // 设置 response 缓存过期，这将用于 Eureka Client 全量获取注册表信息
     ...
     * 服务下线
     *
     * 清除注册表中关 于本应用的租约，避免无效的服务调用。
     *
     * 租约不存在，返回下线失败;如果租约存在，从 registry注册 表中移除， 设置租约的下线时间，同时在最近租约变更记录队列中添加新的下线记录，
     * 以用于 Eureka Client 的增量式获取注册表信息， 最后设置 response 缓存过期 。
     */
    protected boolean internalCancel(String appName, String id, boolean isReplication) {
        read.lock();        // 获取读锁，防止被其他线程进行修改
        try {
            CANCEL.increment(isReplication);
            Map<String, Lease<InstanceInfo>> gMap = registry.get(appName);      // 根据 appName 获取服务实例的集群
            Lease<InstanceInfo> leaseToCancel = null;
            if (gMap != null) {     // 移除服务实例的租约
            ...
            if (leaseToCancel == null) {        // 租约不存在，返回 false
                CANCEL_NOT_FOUND.increment(isReplication);
                logger.warn("DS: Registry: cancel failed because Lease is not registered for: {}/{}", appName, id);
                return false;
            } else {
                leaseToCancel.cancel();     // 设置租约的下线时间
                InstanceInfo instanceInfo = leaseToCancel.getHolder();
                String vip = null;
                String svip = null;
                if (instanceInfo != null) {
                    //添加最近租约变更记录队列，标识 ActionType 为 DELETED
                ...
                invalidateCache(appName, vip, svip);        // 设置 response 缓存过期
     ...
     * 处理心跳请求的核心逻辑
     *
     * 在 #renew 方法中，不关注 Instancelnfo，仅关注于租约本身以及租约的服务实例状态。 如果根据服务实例的 appName 和 instancelnfold 查询
     * 出服务实例的租约，并且根据 OverriddenlnstanceStatus 方法得到的 instanceStatus 不为 InstanceStatus.UNKNOWN,
     * 那么更新租约中的有效时间，即更新租约 Lease 中的 lastUpdateTimestamp，达到续约的目的; 如果租约不存在，那么返回续租失败的结果 。
     */
    public boolean renew(String appName, String id, boolean isReplication) {
        RENEW.increment(isReplication);
        Map<String, Lease<InstanceInfo>> gMap = registry.get(appName);  // 根据 appName 获取服务集群的租约集合
        Lease<InstanceInfo> leaseToRenew = null;
        if (gMap != null) {
            leaseToRenew = gMap.get(id);
        }
        if (leaseToRenew == null) {     // 租约不存在， 直接返回 false
            ...
            if (instanceInfo != null) {
                ...
                if (overriddenInstanceStatus == InstanceStatus.UNKNOWN) {
                    ...
                    RENEW_NOT_FOUND.increment(isReplication);       // 如果得到的服务实例最后状态是 UNKNOWN ， 取消续约
                    return false;
                }
            ...
            renewsLastMin.increment();      //统计每分钟续租的次数 ， 用于自我保护
            leaseToRenew.renew();       //更新租约中的有效时间
            return true;
        }
    }
     ...
     *
     * 如果 Eureka Client在注册后，既没有续约 ，也没有下线(服务崩溃或者网络异常等原 因)，那么服务的状态就处于不可知的状态，不能保证能够从该
     * 服务实例中获取到回馈， 所 以需要服务剔除 AbstractlnstanceRegistry#evict方法定时清理这些不稳定 的服务 ，该方法会 批量将注册表中所有过期租约剔除 。
     */
    @Override
    public void evict() {
        evict(0l);
    ...
    public void evict(long additionalLeaseMs) {
        ...
        if (!isLeaseExpirationEnabled()) {      // 自我保护相关，如果出现该状态，不允许剔除服务
        ...
        List<Lease<InstanceInfo>> expiredLeases = new ArrayList<>();        // 遍历注册表 register， 一次性获取所有的过期租约
        ...
        int registrySize = (int) getLocalRegistrySize();        // 计算最大允许剔除的租约的数量 ， 获取注册表租约总数
        int registrySizeThreshold = (int) (registrySize * serverConfig.getRenewalPercentThreshold());   // 计算注册表租约的阀值， 与自我保护相关
        int evictionLimit = registrySize - registrySizeThreshold;   // 计算剔除租约的数量
        ...
        if (toEvict > 0) {
            ...
            for (int i = 0; i < toEvict; i++) {     //逐个随机剔除
                ...
                internalCancel(appName, id, false);     //逐个剔除
            }
        }
    }
    ...
    public Applications getApplicationsFromMultipleRegions(String[] remoteRegions) {
        ...
        apps.setVersion(1L);
        //从本地 registry获取所有的服务实例信息工 nstanceinfo
        ...
        if (includeRemoteRegion) {      // 获取远程 Region 中的 Eureka Server中的注册表信息
    ...
    public Applications getApplicationDeltasFromMultipleRegions(String[] remoteRegions) {
        ...
        write.lock();       //开启写锁
        try {
            //遍历 recentlyChangedQueue 队列获取最近变化的服务实例信息工 nstanceinfo
            Iterator<RecentlyChangedItem> iter = this.recentlyChangedQueue.iterator();
