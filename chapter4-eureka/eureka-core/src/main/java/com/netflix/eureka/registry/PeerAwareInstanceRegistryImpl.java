 ...
 *
 * 继承了 AbstractinstanceRegistry 的实现，在对本地注册表操作的基础上添加了对其 peer 节点的同步复制操作，使得 Eureka Server 集群中的注册表信息保持一致。
 */
@Singleton
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
     ...
     * 初始化本地注册表信息
     *
     * 在 Eureka Server 启动的过程中， 会从它的 peer 节点中拉取注册表来初始化本地注册表
     */
    @Override
    public int syncUp() {
        // Copy entire entry from neighboring DS node
        int count = 0;      // 从临近的peer中复制整个注册表

        for (int i = 0; ((i < serverConfig.getRegistrySyncRetries()) && (count == 0)); i++) {   // 如果获取不到，线程等待
            if (i > 0) {
                try {
            ...
            Applications apps = eurekaClient.getApplications();     // 获取所有的服务实例
            for (Application app : apps.getRegisteredApplications()) {
                for (InstanceInfo instance : app.getInstances()) {
                    try {
                        if (isRegisterable(instance)) {     // 判断是否可注册，主要用于AWS环境下进行，若部署在其他的环境，直接返回 true
                            register(instance, instance.getLeaseInfo().getDurationInSecs(), true);  // 注册到自身的注册表中
                            count++;
                        }
                    } catch (Throwable t) {
    ...
    public void openForTraffic(ApplicationInfoManager applicationInfoManager, int count) {
        ...
        this.expectedNumberOfClientsSendingRenews = count;      //初始化自我保护机制统计参数
        updateRenewsPerMinThreshold();
        logger.info("Got {} instances from neighboring DS node", count);
        logger.info("Renew threshold is: {}", numberOfRenewsPerMinThreshold);
        this.startupTime = System.currentTimeMillis();
        if (count > 0) {        // 如果同步的应用实例数量为 0，将在一段时间内拒绝 Client 获取注册信息
            this.peerInstancesTransferEmptyOnStartup = false;
        }
        DataCenterInfo.Name selfName = applicationInfoManager.getInfo().getDataCenterInfo().getName();
        boolean isAws = Name.Amazon == selfName;
        if (isAws && serverConfig.shouldPrimeAwsReplicaConnections()) {     // 判断是否是AWS运行环境 ，此处忽略
            logger.info("Priming AWS connections for all replicas..");
            primeAwsReplicas(applicationInfoManager);
        }
        logger.info("Changing status to UP");
        applicationInfoManager.setInstanceStatus(InstanceStatus.UP);        // 修改服务实例的状态为健康上线，可以接受流量
        super.postInit();
    }
    ...
    public void register(final InstanceInfo info, final boolean isReplication) {
        ...
        // 对 Abstractinstancregistry 中的 #register、 #cancel 和 #renew 等方法都添加了 同步到 peer节点的操作，使 Server集群中注册表信息保持最终一致性
    ...
    private void replicateToPeers(Action action, String appName, String id,
        ...
        try {
            ...
            //向 peer集群中的每一个 peer进行同步
            for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
                // If the url represents this host, do not replicate to yourself.
                if (peerEurekaNodes.isThisMyUrl(node.getServiceUrl())) {        // 如果peer节点是自身的话， 不进行同步复和l
                    continue;
                }
                replicateInstanceActionsToPeers(action, appName, id, info, newStatus, node);    // 根据 Action调用不同的同步请求
            }
        } finally {
            tracer.stop();
        }
     ...
     *
     * 在进行同步复 制的时候根据任 务编号 合并操作，减少同步操作的数 量 和网络消耗，但是同时也造成同 步复 制的延时性，不 满足 CAP 中的 c (强一致性)。
     */
        ...
        try {
            ...
            switch (action) {
                ...
                case Heartbeat:
                    ...
                    node.heartbeat(appName, id, infoFromRegistry, overriddenStatus, false); //同步心跳
                    break;
                case Register:
                    node.register(info);        //同步注册
                    break;
                case StatusUpdate:
                    infoFromRegistry = getInstanceByAppAndId(appName, id, false);
                    node.statusUpdate(appName, id, newStatus, infoFromRegistry);        //同步状态更新
                    break;
